        ...
        <div class="flex-1">
            <div class="flex justify-between items-center">
                <div>
                    ...
                    <small
                        class="ml-2 text-sm text-gray-600">{{ $comment->created_at->diffForHumans() }}</small>
                    ...
                    <x-dropdown>
                        ...
                        <x-slot name="content">
                            ...
                            <form method="POST" action="{{ route('chirps.destroy', $comment) }}">
                                @csrf
                                @method('delete')
                                <x-dropdown-link :href="route('chirps.destroy', $comment)" onclick="event.preventDefault(); this.closest('form').submit();">
                                    {{ __('Delete') }}
                                </x-dropdown-link>
                            </form>
                        </x-slot>
